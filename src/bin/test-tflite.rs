...
impl LanguageModel for &Dict {
    ...
    fn odds(&self, input: &str, ch: char) -> f32 {
        ...
        let word_start =
            input.rfind(|ch| " .,".contains(ch)).map(|i| i+1).unwrap_or(0);
        ...
        if words.range(prefix2..).next().is_some() {
            let extended = format!("{}{}", prefix, ch);
            if words.range(extended..).next().is_some() {
                Dict::VALID
            }
            else {
                Dict::INVALID
...
fn main() {
    ...
    while let Some(Ok(line)) = lines.next() {
        ...
        let actual = recognizer.recognize(&ink.as_slice(), &ml::Beam { size: 4, language_model: &lm }).unwrap();

        let cer = strsim::levenshtein(expected, &actual[0].0) as f32 / expected.len() as f32;
        global_err += cer;
        count += 1;

        println!("[{:.4}] {} -> {}", cer, expected, actual[0].0);
    }
